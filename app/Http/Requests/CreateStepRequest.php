...
class CreateStepRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'steps.*.media_urls.*' => 'sometimes',
            'steps.media_titles.*' => 'required_if:steps.media_urls.*,required_if:steps.media_files.*,string|max:' . config('constants.MAX_STRING_LENGTH'),
            'steps.*.quiz' => 'sometimes|array',
            'steps.*.quiz.title' => 'required_with:steps.*.quiz|string|max:255',
    ...
    public function messages(): array
    {
        return [
            ...
            'steps.*.media_urls.*.required' => __('messages.step_media_urls_required'),
            'steps.*.media_titles.*.required_if' => __('messages.step_media_titles_required_if'),
            'steps.*.quiz.required' => __('messages.step_quiz_required'),
            'steps.*.quiz.title.required_with' => __('messages.step_quiz_title_required_with'),
            'steps.*.quiz.title.string' => __('messages.step_quiz_title_string'),
            'steps.*.quiz.title.max' => __('messages.step_quiz_title_max'),
